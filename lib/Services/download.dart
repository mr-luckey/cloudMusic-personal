...
import 'package:blackhole/Helpers/performance_helper.dart';
...
class Download with ChangeNotifier {
  ...
  // Add StreamSubscription to properly cancel downloads
  ...
  Future<void> prepareDownload(
    ...
    try {
      ...
      if (dlPath == '') {
        ...
        if (temp == null) {
          Logger.root.severe('Failed to get storage path');
          return;
        }
        dlPath = temp;
      }
                          ...
                          children: [
                            ...
                            TextButton(
                              ...
                              onPressed: () async {
                                Navigator.pop(context);
                                while (
                                    await File('$dlPath/$filename').exists()) {
                                  filename =
      ...
      Logger.root.severe('Error in prepareDownload', e, stackTrace);
    ...
    try {
      ...
      if (isYouTubeSong) {
        try {
          ...
          notifyListeners();
          try {
            if (filepath != null) await File(filepath!).delete();
        ...
        try {
          ...
          Logger.root.severe('Error connecting to JioSaavn', e, stackTrace);
      ...
      _downloadSubscription = stream.listen((value) {
        bytes.addAll(value);
        try {
          recieved += value.length;
          final newProgress = recieved / total;

          // Throttle progress updates to ~60fps to avoid excessive UI rebuilds
          PerformanceHelper.throttle(
            action: () {
          ...
          if (!download) {
            ...
            // Close HTTP client if it exists (for JioSaavn)
            if (client != null) {
              client.close();
            }

            // Perform cleanup immediately (onDone won't be called after cancel)
            Future.microtask(() async {
              download = true; // Reset flag
              progress = 0.0;
              ...
              try {
                ...
                final imageFile = File(filepath2);
                if (await imageFile.exists()) {
                  await imageFile.delete();
                }
              } catch (e) {
                Logger.root.warning('Error deleting files: $e');
              }
      ...
      // Register onDone callback separately to avoid void type error
      ...
      Logger.root.severe('Error in downloadSong', e, stackTrace);
